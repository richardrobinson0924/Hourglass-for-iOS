...
struct UIViewWrapper<V>: UIViewRepresentable where V : UIView {
    @Binding var view: V
...
struct ConfettiView_Previews: PreviewProvider {
    @State static var uiView = ConfettiUIView()
    
    static var previews: some View {
        UIViewWrapper(view: $uiView).edgesIgnoringSafeArea(.all).onTapGesture {
            uiView.emit(with: [.shape(.circle, .blue)])
        }
    }
}
